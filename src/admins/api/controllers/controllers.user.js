...
import { SAVE_ADMIN_UPLOADED_DOCUMENT_CONTROLLER, UPDATE_BANK_STMT_RECORD } from '../../../users/lib/enums/lib.enum.labels';
...
export const sendNotifications = async (req, res, next) => {
  try {
    ...
    if (type === 'incomplete-profile') {
      ...
      if (
        userDetails.is_completed_kyc &&
        userDetails.is_verified_bvn &&
        userDetails.is_uploaded_identity_card &&
        userEmploymentDetails?.monthly_income &&
        userDetails?.number_of_children &&
        userDetails?.marital_status &&
        userEmploymentDetails?.employment_type
      ) {
...
export const fetchUsers = async (req, res, next) => {
  try {
    ...
    const [users, [usersCount]] = await Promise.all([processAnyData(userQueries.fetchUsers, payload), processAnyData(userQueries.fetchUsersCount, payload)]);
  ...
  try {
    ...
    if (body.decision === 'decline') {
      ...
      sendUserPersonalNotification(
        userDetails,
        `${userDetails.first_name} declined utility bill`,
        PersonalNotifications.declinedUtilityBillNotification(),
        'utility-declined',
        {}
      );
    ...
    sendUserPersonalNotification(
      userDetails,
      `${userDetails.first_name} approved utility bill`,
      PersonalNotifications.approvedUtilityBillNotification(),
...
export const updateUserProfile = async (req, res, next) => {
  try {
    ...
    await adminActivityTracking(
      ...
      75,
      'fail',
      descriptions.update_user_details_failed(`${req.admin.first_name}, ${req.admin.last_name}`, req.userDetails.name)
    );
