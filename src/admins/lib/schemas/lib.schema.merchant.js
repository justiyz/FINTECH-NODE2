...
const createMerchant = Joi.object().keys({
  ...
  processing_fee: Joi.number().positive().min(0).required().label('Processing fee'),
  insurance_fee: Joi.number().positive().min(0).required().label('Insurance fee'),
  advisory_fee: Joi.number().positive().min(0).required().label('Advisory fee'),
  customer_loan_max_amount: Joi.number().positive().min(0).required().label('Limit per customer'),
  merchant_loan_limit: Joi.number().positive().min(0).required().label('Merchant loan limit'),
...
const updateMerchant = Joi.object().keys({
  ...
  interest_rate: Joi.number().optional().label('Interest Rate'),
  orr_score_threshold: Joi.string().optional().regex(/^\d+$/).messages({
    'string.pattern.base': 'ORR score must be a number',
  }).label('ORR score'),
  processing_fee: Joi.number().positive().min(0).optional().label('Processing fee'),
  insurance_fee: Joi.number().positive().min(0).optional().label('Insurance fee'),
  advisory_fee: Joi.number().positive().min(0).optional().label('Advisory fee'),
  customer_loan_max_amount: Joi.number().positive().min(0).optional().label('Limit per customer'),
  merchant_loan_limit: Joi.number().positive().min(0).optional().label('Merchant loan limit'),
