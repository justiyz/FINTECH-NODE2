...
export const checkSeedfiPaystackBalance = async(req, res, next) => {
  try {
    ...
    if (result.response.status !== 200) {
      ...
      return ApiResponse.error(res, result.response.data.message, enums.HTTP_SERVICE_UNAVAILABLE, enums.CHECK_SEEDFI_PAYSTACK_BALANCE_MIDDLEWARE);
    }
  } catch (error) {
    userActivityTracking(req.user.user_id, 44, 'fail');
...
export const generateLoanDisbursementRecipient = async(req, res, next) => {
  try {
    ...
    if (result.response.status !== 200) {
      userActivityTracking(user.user_id, 44, 'fail');
      return ApiResponse.error(res, result.response.data.message, enums.HTTP_SERVICE_UNAVAILABLE, enums.GENERATE_LOAN_DISBURSEMENT_RECIPIENT_MIDDLEWARE);
    }
    userActivityTracking(req.user.user_id, 44, 'fail');
