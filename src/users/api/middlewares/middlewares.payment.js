...
export const ticketPurchaseUpdate = async (req, res, next) => {
  try {
    const {
      body: { ticket_id, user_id, reference },
      user,
    } = req;
    ...
    await processOneOrNoneData(loanQueries.updatePersonalLoanPaymentTable, [
      user_id,
      repaymentRecord.loan_id,
      repaymentRecord.principal_payment,
      'debit',
      'ticket loan',
      'part loan repayment',
      'paystack',
    ...
    const ticketUrls = ticket_record
      .map(ticket => `<a href="${ticket.ticket_url}" style="text-decoration: underline; color: blue; cursor: pointer;">Click here for your ticket [].</a><br/>`)
      .join('');
    ...
    if (body.event === 'refund.failed' || body.event === 'refund.pending' || body.event === 'refund.processed' || body.event === 'refund.processing') {
      ...
      if (body.data.status === 'processed') {
        ...
        sendUserPersonalNotification(
          user,
          'Card Tokenization amount refund completed',
          PersonalNotifications.cardTokenizationAmountRefundCompletelyProcessed(),
          'card-tokenization-refund-processed',
          {}
        );
      ...
      if (body.data.status === 'failed') {
        ...
        sendUserPersonalNotification(
          user,
          'Card Tokenization amount refund failed',
          PersonalNotifications.cardTokenizationAmountRefundProcessingFailed(),
          'card-tokenization-refund-failed',
          {}
        );
...
export const saveCardAuth = async (req, res, next) => {
  try {
    ...
    if (body.event === 'charge.success' && body.data.channel === 'card') {
      ...
      if (
        paymentRecord.payment_type === 'card_tokenization' &&
        ...
        sendUserPersonalNotification(user, 'Card Tokenization fails', PersonalNotifications.cardTokenizationFailedDueToCardExpiration(), 'failed-card-tokenization', {});
      ...
      if (
        paymentRecord.payment_type === 'card_tokenization' &&
        body.data.authorization.account_name?.trim().length > 0 &&
        ...
        logger.info(`${enums.CURRENT_TIME_STAMP}, ${paymentRecord.user_id}:::Info: successfully sends mail to the user saveCardAuth.middlewares.payment.js`);
        sendUserPersonalNotification(user, 'Card Tokenization fails', PersonalNotifications.cardTokenizationFailedDueToNotCardHolderName(), 'failed-card-tokenization', {});
      ...
      if (cardPreviouslySaved) {
        ...
        await processAnyData(paymentQueries.updateUserCardAuthToken, [
          ...checkIfCardPreviouslyUsedPayload,
      ...
      sendUserPersonalNotification(user, 'Card Tokenization successful', PersonalNotifications.cardTokenizedSuccessfully(), 'successful-card-tokenization', {});
  ...
  try {
    ...
    if (body.event === 'charge.success' && paymentRecord.payment_type === 'card_tokenization') {
      ...
      if (result.message.toLowerCase() === 'refund has been queued for processing') {
        ...
        sendUserPersonalNotification(
          user,
          'Card Tokenization amount refund initiated',
          PersonalNotifications.cardTokenizationAmountRefundInitiated(),
          'successful-card-tokenization-refund-initiation',
          {}
        );
      ...
      sendUserPersonalNotification(
        user,
        'Card Tokenization amount refund failed to be initiated',
        PersonalNotifications.cardTokenizationAmountRefundCouldNotBeInitiated(),
        'failed-card-tokenization-refund-initiation',
        {}
      );
  ...
  try {
    ...
    if (body.event.includes('transfer') && paymentRecord.payment_type === 'personal_loan_disbursement') {
      ...
      if (body.event === 'transfer.success') {
        ...
        const loanMandate = await processOneOrNoneData(loanMandateQueries.getLoanMandateByLoanId, [loanDetails.loan_id]);

        let repaymentSchedule;

        if (!loanMandate) {
          repaymentSchedule = await generateLoanRepaymentSchedule(loanDetails, paymentRecord.user_id);
        } else {
          repaymentSchedule = await processAnyData(loanQueries.fetchLoanRepaymentScheduleForMandate, [loanDetails.loan_id, user.user_id]);
        }
        ...
        const [rewardRangeDetails] = await processAnyData(authQueries.fetchLoanRequestPointDetailsBasedOnAmount, [rewardDetails.reward_id, parseFloat(paymentRecord.amount)]);
        ...
        sendUserPersonalNotification(
          ...
          PersonalNotifications.loanDisbursementSuccessful({ amount: parseFloat(paymentRecord.amount) }),
          'successful-disbursement',
          {}
        );
        sendPushNotification(userDetails.user_id, PushNotifications.successfulLoanDisbursement(), userDetails.fcm_token);
        admins.map(admin => {
          sendNotificationToAdmin(admin.admin_id, 'Loan Disbursement', adminNotification.loanDisbursement(), [`${user.first_name} ${user.last_name}`], 'loan-disbursement');
        });
...
export const processPersonalLoanRepayments = async (req, res, next) => {
  try {
    const { body, paymentRecord } = req;
    if (
      (body.event === 'charge.success' && (paymentRecord.payment_type === 'part_loan_repayment' || paymentRecord.payment_type === 'full_loan_repayment')) ||
      ...
      if (paymentRecord.payment_type === 'part_loan_repayment' || paymentRecord.payment_type === 'automatic_loan_repayment') {
        ...
        let customRepaymentCompleted =
          paymentRecord.payment_type == 'part_loan_repayment' && parseFloat(paymentRecord.amount) >= parseFloat(nextRepayment.post_payment_outstanding_amount);
        ...
        await Promise.all([
          ...
          customRepaymentCompleted
            ? processAnyData(loanQueries.updateNextLoanRepayment, [nextRepayment.loan_repayment_id, parseFloat(paymentRecord.amount)])
            : processAnyData(loanQueries.updateNextLoanCustomRepayment, [nextRepayment.loan_repayment_id, parseFloat(paymentRecord.amount)]),
        ...
        await MailService('Successful loan repayment', 'successfulRepayment', {
          ...user,
          amount_paid: parseFloat(paymentRecord.amount).toFixed(2),
          total_loan_amount: parseFloat(loanDetails.total_repayment_amount).toFixed(2),
        });
        sendUserPersonalNotification(
          user,
          'Part loan repayment successful',
          PersonalNotifications.partLoanRepaymentSuccessful({ amount: parseFloat(paymentRecord.amount) }),
        ...
        if (statusType === 'completed') {
          ...
          sendUserPersonalNotification(
            user,
            'Full loan repayment successful',
            PersonalNotifications.fullLoanRepaymentSuccessful({ amount: parseFloat(paymentRecord.amount) }),
            'successful-repayment',
            {}
          );
          await MailService('Loan successfully repaid', 'completedRepayment', {
            ...user,
            loan_reason: loanDetails.loan_reason,
            total_loan_amount: parseFloat(loanDetails.amount_requested).toFixed(2),
            loan_duration: loanDetails.reschedule_loan_tenor_in_months || `${loanDetails.loan_tenor_in_months} months`,
            interest_rate: loanDetails.percentage_pricing_band,
            total_repayment: loanDetails.total_repayment_amount,
      ...
      await Promise.all([
        ...
        processAnyData(loanQueries.updateLoanWithRepayment, [
          paymentRecord.loan_id,
          paymentRecord.user_id,
          'completed',
          parseFloat(paymentRecord.amount),
  ...
  try {
    ...
    if (body.event.includes('transfer') && paymentRecord.payment_type === 'cluster_loan_disbursement') {
      ...
      if (body.event === 'transfer.success') {
        ...
        await processAnyData(clusterQueries.updateGeneralClusterLoanDetails, [
          generalClusterLoanDetails.cluster_id,
          parseFloat(clusterTotalLoanObligation.cluster_total_outstanding_amount),
          parseFloat(generalClusterLoanDetails.total_amount_requested),
        ]);
        ...
        await qualifiedClusterMembers.map(async member => {
          ...
          sendUserPersonalNotification(
            userDetails,
            'Loan disbursement successful',
            PersonalNotifications.loanDisbursementSuccessful({ amount: parseFloat(member.amount_requested) }),
            'successful-disbursement',
            {}
          );
          sendPushNotification(userDetails.user_id, PushNotifications.successfulLoanDisbursement(), userDetails.fcm_token);
        });
        sendClusterNotification(paymentRecord, cluster, { is_admin: true }, 'Cluster loan successfully disbursed', 'loan-application-disbursed', {
          ...generalClusterLoanDetails,
        });
        userActivityTracking(paymentRecord.user_id, 42, 'success');
        return ApiResponse.success(res, enums.BANK_TRANSFER_SUCCESS_STATUS_RECORDED, enums.HTTP_OK);
      }
      if (body.event === 'transfer.failed') {
        ...
        await processAnyData(loanQueries.initializeBankTransferPayment, [
          paymentRecord.user_id,
          generalClusterLoanDetails.total_amount_requested,
          'paystack',
          reference,
          'cluster_loan_disbursement',
          'requested cluster loan facility disbursement',
          paymentRecord.loan_id,
        ]);
        logger.info(`${enums.CURRENT_TIME_STAMP}, ${paymentRecord.user_id}:::Info: loan payment re-initialized in the DB
        processClusterLoanTransferPayments.middlewares.payment.js`);
...
export const processClusterLoanRepayments = async (req, res, next) => {
  try {
    const { body, paymentRecord } = req;
    if (
      (body.event === 'charge.success' && (paymentRecord.payment_type === 'part_cluster_loan_repayment' || paymentRecord.payment_type === 'full_cluster_loan_repayment')) ||
      ...
      if (paymentRecord.payment_type === 'part_cluster_loan_repayment' || paymentRecord.payment_type === 'automatic_cluster_loan_repayment') {
        ...
        const otherExistingOverDueRepayments = await processAnyData(clusterQueries.fetchOtherLoanOverDueRepayments, [
          clusterLoanDetails.loan_id,
          nextClusterLoanRepayment.loan_repayment_id,
        ]);
        ...
        await Promise.all([
          processAnyData(clusterQueries.updateClusterLoanPaymentTable, [
            paymentRecord.user_id,
            clusterLoanDetails.cluster_id,
            clusterLoanDetails.member_loan_id,
            clusterLoanDetails.loan_id,
            parseFloat(paymentRecord.amount),
            'debit',
            `${clusterLoanDetails.cluster_name} cluster loan`,
            paymentDescriptionType,
            `paystack ${body.data.channel}`,
          ]),
          processAnyData(clusterQueries.updateNextClusterLoanRepayment, [nextClusterLoanRepayment.loan_repayment_id]),
          processAnyData(clusterQueries.updateClusterLoanWithRepayment, [
            clusterLoanDetails.member_loan_id,
            paymentRecord.user_id,
            statusType,
            parseFloat(paymentRecord.amount),
          ...
          processAnyData(clusterQueries.updateClusterMemberWithRepayment, [
            clusterLoanDetails.cluster_id,
            paymentRecord.user_id,
            clusterMemberLoanStatus,
            parseFloat(paymentRecord.amount),
          ]),
        ...
        await MailService('Successful loan repayment', 'successfulRepayment', {
          ...user,
          amount_paid: parseFloat(paymentRecord.amount).toFixed(2),
          total_loan_amount: parseFloat(clusterLoanDetails.amount_requested).toFixed(2),
        });
        sendUserPersonalNotification(
          user,
          'Part cluster loan repayment successful',
          PersonalNotifications.partLoanRepaymentSuccessful({ amount: parseFloat(paymentRecord.amount) }),
          'successful-repayment',
          {}
        );
        ...
        sendClusterNotification(
          ...
          `${user.first_name} ${user.last_name} repays part loan payment`,
        ...
        if (statusType === 'completed') {
          ...
          sendUserPersonalNotification(
            user,
            'Full cluster loan repayment successful',
            PersonalNotifications.fullLoanRepaymentSuccessful({ amount: parseFloat(paymentRecord.amount) }),
            'successful-repayment',
            {}
          );
          await MailService('Loan successfully repaid', 'completedRepayment', {
            ...user,
            loan_reason: `${clusterLoanDetails.cluster_name} cluster loan`,
            total_loan_amount: parseFloat(clusterLoanDetails.amount_requested).toFixed(2),
            loan_duration: clusterLoanDetails.reschedule_loan_tenor_in_months || `${clusterLoanDetails.loan_tenor_in_months} months`,
            interest_rate: clusterLoanDetails.percentage_pricing_band,
            total_repayment: clusterLoanDetails.total_repayment_amount,
          ...
          sendClusterNotification(
            ...
            cluster,
            { is_admin: isClusterAdmin },
            `${user.first_name} ${user.last_name} repays full loan payment`,
      ...
      await Promise.all([
        ...
        processAnyData(clusterQueries.updateClusterMemberWithRepayment, [clusterLoanDetails.cluster_id, paymentRecord.user_id, 'inactive', parseFloat(paymentRecord.amount)]),
        processAnyData(clusterQueries.updateGeneralClusterWithRepayment, [clusterLoanDetails.cluster_id, parseFloat(paymentRecord.amount), clusterLoanStatus]),
      ]);
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${paymentRecord.user_id}:::Info: cluster loan repayment and payment details updated successfully
      processClusterLoanRepayments.middlewares.payment.js`);
