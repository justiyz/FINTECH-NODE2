...
import { API_VERSION, BANK_UPDATE_SUCCESSFUL, BVN_INFORMATION_UNAVAILABLE, CREATE_BANK_RECORD, DELETE_BANK_RECORD } from '../../lib/enums/lib.enum.messages';
...
export const updateUserRefreshToken = async (req, res, next) => {
  try {
    ...
    const is_updated_advanced_kyc =
      userEmploymentDetails?.monthly_income && user?.number_of_children && user?.marital_status && userEmploymentDetails?.employment_type ? true : false;
    const data = {
      ...updatedUser,
...
export const idUploadVerification = async (req, res, next) => {
  try {
    ...
    const document = encodeURIComponent(await Hash.encrypt({ document_url: body.image_url.trim(), document_extension: fileExt }));
...
export const checkIfTheLengthOfThePassportNumberIsCorrect = async (passportNumber, user, res, next) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now checking the length/format of the Passport Number checkIfTheLengthOfThePassportNumberIsCorrect.middlewares.user.js`
  );
  ...
  if (!ipnRegex.test(passportNumber)) {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: Passport Number is not in the correct format/length {internationPassportVerification} documentVerification.controller.user.js`
    );
    return false;
  }
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: International Passport Number is in the correct format/length {internationPassportVerification} documentVerification.controller.user.js`
  );
  return true;
};
...
export const checkIfTheLengthOfTheNinIsCorrect = async (nin, user, res, next) => {
  ...
  if (!ninRegex.test(nin)) {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: NIN is not in the correct format/length {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
    return false;
  }
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: NIN is in the correct format/length {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
  );
  return true;
};
...
export const callTheZeehAfricaNINVerificationCheck = async (nin, user) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to verify the user NIN with zeeh africa  {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
  );
  const response = await zeehService.zeehNINVerificationCheck(nin, user);
  if (response.status === 'success') {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user NIN with zeeh africa successful {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  } else {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user NIN with zeeh africa failed {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  }
  return response;
};
...
export const callTheDojahInternationPassportVerificationCheck = async (user_data, user) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to verify the user passport number with dojah  {passportNumberVerification} documentVerification.controller.user.js`
  );
  const response = await dojahService.dojahPassportNumberVerificationCheck(user_data, user);
  if (response.status === 'success') {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user passport number with dojah successful {passportNumberVerification} documentVerification.controller.user.js`
    );
  } else {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user passport number with dojah failed {passportNumberVerification} documentVerification.controller.user.js`
    );
  }
  return response;
};
...
export const callTheZeehAfricaInternationPassportVerificationCheck = async (user, document_id) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to verify the user passport number with zeeh africa  {passportNumberVerification} documentVerification.controller.user.js`
  );
  const response = await zeehService.zeehPassportNumberVerificationCheck(user, document_id);
  if (response.status === 'success') {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user passport number with zeeh africa successful {passportNumberVerification} documentVerification.controller.user.js`
    );
  } else {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user passport number with zeeh africa failed {passportNumberVerification} documentVerification.controller.user.js`
    );
  }
  return response;
};
...
export const callTheDojahNINVerificationCheck = async (nin, user) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to verify the user NIN with dojah  {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
  );
  const response = await dojahService.dojahNINVerification(nin, user);
  if (response.status === 200) {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user NIN with dojah successful {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  } else {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user NIN with dojah failed {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  }
  return response;
};
...
export const uploadImageToS3Bucket = async (user, document, user_data) => {
  try {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to upload image to S3 bucket {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
    ...
    if (data.Location) {
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully uploaded image to S3 bucket {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
      );
    } else {
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: failed to upload image to S3 bucket {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
      );
    }
    return data;
  } catch (error) {
    logger.error(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Error: failed to upload image to S3 bucket ${error.message} {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
    throw error;
  }
};
...
export const internationPassportVerification = async (req, res, next) => {
  ...
  if (!(await checkIfTheLengthOfThePassportNumberIsCorrect(body.document_id, user, res, next))) {
    return ApiResponse.error(
      res,
      'Passport Number must be exactly 9 digits long and consist of numbers and letters',
      enums.HTTP_BAD_REQUEST,
      enums.CHECK_NIN_LENGTH_MIDDLEWARE
    );
  }
  let user_data;
  ...
  if (internationalPassportVerificationResponse.status === 'success' || internationalPassportVerificationResponse.status === 200) {
    if (await checkIfUserDetailsMatchDocumentCheckResponse(user, user_data)) {
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully checked that the user details match the NIN details {internationPassportVerification} documentVerification.controller.user.js`
      );

      const data = await uploadImageToS3Bucket(user, body, user_data);
      const updateIdVerification = [user.user_id, body.document_type, body.document_id, data.Location, null, null, null];

      await processAnyData(userQueries.updateIdVerification, updateIdVerification);
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user_national_id_details created successfully {internationPassportVerification} documentVerification.controller.user.js`
      );

      const fileExt = path.extname(user_data.photo.trim());
      const documen_t = encodeURIComponent(await Hash.encrypt({ document_url: user_data.photo.trim(), document_extension: fileExt }));

      await processAnyData(userQueries.addDocumentTOUserUploadedDocuments, [user.user_id, 'valid identification', documen_t]);
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user_admin_uploaded_documents created successfully {internationPassportVerification} documentVerification.controller.user.js`
      );
...
export const nationalIdentificationNumberVerification = async (document, user, res, next) => {
  ...
  if (ninResponse.status === 'success' || ninResponse.status === 200) {
    if (await checkIfUserDetailsMatchNinResponse(user_data, user)) {
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully checked that the user details match the NIN details {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
      );
      logger.info(`${document} ${user_data}`);
      const data = await uploadImageToS3Bucket(user, document, user_data);
      const updateIdVerification = [user.user_id, document.document_type, document.document_id, data.Location, null, null, null];

      await processAnyData(userQueries.updateIdVerification, updateIdVerification);
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user_national_id_details created successfully {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
      );
      ...
      const [response] = await processAnyData(userQueries.userIdentityVerification, [user.user_id, data.Location, tierChoice]);
      logger.info(
        `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user details updated successfully {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
      );
...
export const callTheZeehAfricaVINVerificationCheck = async (vin, user, state) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to verify the the user VIN with zeeh africa  {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
  );
  const response = await zeehService.zeehVINVerificationCheck(vin, user, state);
  if (response.status === 'success') {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user VIN verification with zeeh africa successful {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  } else {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user VIN verification with zeeh africa failed {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  }
  return response;
};
...
export const callTheDojahVINVerificationCheck = async (vin, user) => {
  logger.info(
    `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: now trying to verify the user VIN with dojah  {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
  );
  const response = await dojahService.dojahNINVerification(vin, user);
  if (response.status === 200) {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user VIN with dojah successful {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  } else {
    logger.info(
      `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: verifying user VIN with dojah failed {nationalIdentificationNumberVerification} documentVerification.controller.user.js`
    );
  }
  return response;
};
  ...
  try {
    ...
    if (vinResponse.status === 'success' || vinResponse.status === 200) {
      if (checkIfUserDetailsMatchNinResponse(user_data, user)) {
        logger.info(
          `${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully checked that the user details match the VIN details {votersIdentificationNumberVerification} documentVerification.controller.user.js`
        );
...
export const initiateAddressVerification = async (req, res) => {
  try {
    ...
    const candidateId =
      userAddressDetails && userAddressDetails.you_verify_candidate_id !== null ? userAddressDetails.you_verify_candidate_id : userYouVerifyCandidateDetails.id;
...
export const updateUploadedUtilityBill = async (req, res, next) => {
  try {
    ...
    await Promise.all([
      !userAddressDetails
        ? processOneOrNoneData(userQueries.addUserUtilityBillDocument, [user.user_id, document])
  ...
  try {
    ...
    if (body.data.taskStatus.toLowerCase() === 'verified') {
      ...
      sendUserPersonalNotification(
        userDetails,
        `${userDetails.first_name} address verification successful`,
        PersonalNotifications.userAddressVerificationSuccessful(userDetails, userAddressDetails),
    ...
    updateUserAddressVerificationStatus.controller.user.js`);
      ...
      if (tierChoice === '2') {
        ...
        sendUserPersonalNotification(
          ...
          PersonalNotifications.tierUpgradedSuccessfully(userDetails.first_name),
          'tier-upgraded-successfully',
          {}
        );
      }
    ...
    sendUserPersonalNotification(
      userDetails,
      `${userDetails.first_name} address verification failed`,
      PersonalNotifications.userAddressVerificationFailed(userDetails, userAddressDetails),
...
export const userClaimsReferralPoints = async (req, res, next) => {
  try {
    ...
    await MailService('Reward points claimed', 'rewardPointsClaiming', {
      ...user,
      just_claimed_points: referralDetails.unclaimed_reward_points,
...
export const verifyBvnOtp = async (req, res, next) => {
  try {
    ...
    return ApiResponse.success(res, enums.VERIFIED('OTP code'), enums.HTTP_OK, {
      ...
      phone_number: data.data.phoneNumber1,
