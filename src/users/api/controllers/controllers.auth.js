...
export const completeProfile = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: check if user was referred to the platform returns completeProfile.controllers.auth.js`);
    if (userWasReferred) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user was referred to the platform completeProfile.controllers.auth.js`);
      ...
      sendUserPersonalNotification(referenceUserDetails, 'Welcome point', 
        PersonalNotifications.inviterUserPlatformUserReferralPointMessage(rewardPoint, 'welcome'), 'referral-rewards', {});
      userActivityTracking(referenceUserDetails.user_id, 102, 'success');
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: referred user awarded referral reward point and notified completeProfile.controllers.auth.js`);
    }
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user was not referred to the platform completeProfile.controllers.auth.js`);
