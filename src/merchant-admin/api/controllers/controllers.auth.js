...
export const completeAdminLoginRequest = async(req, res, next) => {
  try {
    ...
    await adminActivityTracking(req.admin.merchant_admin_id, 9, 'success', descriptions.login_request(adminName));
    ...
    await adminActivityTracking(req.admin.merchant_admin_id, 9, 'fail', descriptions.login_request_failed(req.admin.first_name));
...
export const login = async(req, res, next) => {
  try {
    const { admin } = req;
    const adminName = `${admin.first_name} ${admin.last_name}`;
    const token = await Hash.generateMerchantAdminAuthToken(admin);
    ...
    await adminActivityTracking(req.admin.merchant_admin_id, 10, 'fail', descriptions.login_approved_failed(req.admin.first_name));
...
export const setPassword =  (type = '') => async(req, res, next) => {
  try {
    ...
    await adminActivityTracking(req.admin.merchant_admin_id, typeMonitor, 'success', description);
    return ApiResponse.success(res, enums.PASSWORD_SET_SUCCESSFULLY, enums.HTTP_OK, setNewPassword);
  } catch (error) {
    await adminActivityTracking(req.admin.merchant_admin_id, 'fail', descriptions.new_password_failed());
