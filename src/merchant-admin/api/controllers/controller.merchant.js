...
export const forgotMerchantPassword = async(req, res, next) => {
  try {
    ...
    await adminActivityTracking(req.admin.merchant_admin_id, 1, 'success', descriptions.forgot_password());
    ...
    await adminActivityTracking(req.admin.merchant_admin_id, 1, 'fail', descriptions.forgot_password_failed());
...
//       });
//     }
//     logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: merchant successfully created createMerchant.admin.controller.merchant.js`);
...
//     logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Complete request CreateMerchant.admin.controllers.merchant.js`);
//     return ApiResponse.success(
//       res,
...
export const fetchMerchants = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Calculated offset for fetching merchants fetchMerchants.admin.controllers.merchant.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Successfully fetched merchants from the DB fetchMerchants.admin.controllers.merchant.js`);
    const count = merchants.length !== 0 ? merchants[0].total : 0;
    if (Array.isArray(merchants)) {
      merchants.forEach((item) => delete item.total);
    }
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Format result for request fetchMerchants.admin.controllers.merchant.js`);
    const result =  {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Complete request to fetch all merchants fetchMerchants.admin.controllers.merchant.js`);
    return ApiResponse.success(
      res,
...
export const fetchMerchantLoans = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: successfully counted merchant loans from the DB fetchMerchantLoans.admin.controllers.merchant.js`);
    const payload = MerchantPayload.fetchMerchantLoans(req);
    const loans = await processAnyData(
      merchantQueries.fetchMerchantLoans,
      payload
    );
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: successfully fetched merchant loans from the DB fetchMerchantLoans.admin.controllers.merchant.js`);
    const result = {
...
export const fetchUserCreditScoreBreakdown = async(req, res, next) => {
  try {
    ...
    if (result.status !== 200 || !(result.data?.credit_score)) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${admin.merchant_admin_id}:::Info: error occured fetching user credit score breakdown from creditscoring service fetchUserCreditScoreBreakdown.admin.controllers.merchant.js`);
      return ApiResponse.error(
        res,
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${admin.merchant_admin_id}:::Info: successfully retreived creditscore breakdown from creditscoring service fetchUserCreditScoreBreakdown.admin.controllers.merchant.js`);
    return ApiResponse.success(
      res,
...
export const fetchAvailableBankList = async(req, res, next) => {
  try {
    ...
    logger.info(`${ enums.CURRENT_TIME_STAMP }, ${ admin.merchant_admin_id }:::Info: bank lists returned from paystack fetchAvailableBankList.controller.merchant.js`);
    return ApiResponse.success(
      res,
      data.message,
      enums.HTTP_OK,
...
export const resolveBankAccountNumber = async(req, res, next) => {
  try {
    ...
    if (data?.status !== true) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${admin.merchant_admin_id}:::Info: account number not resolved successfully by paystack resolveBankAccountNumberName.admin.controllers.merchant.js`);
      return ApiResponse.error(
        res,
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${admin.merchant_admin_id}:::Info: account number resolved successfully by paystack resolveBankAccountNumberName.admin.controllers.merchant.js`);
    return ApiResponse.success(
      res,
      data.message,
      enums.HTTP_OK,
...
export const updateMerchant = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: confirm that merchant details has been edited and updated in the DB. updateMerchant.admin.controllers.merchant.js`);
...
const addMerchantBankAccount = async(admin, payload, newAccount = false) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: initiate request to generate recipient code addMerchantBankAccount.admin.controllers.merchant.js`);
    ...
    if (!existingBankAccount) {
      ...
      logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Merchant bank account created successfully addMerchantBankAccount.admin.controllers.merchant.js`);
    } else {
      ...
      logger.info(`${enums.CURRENT_TIME_STAMP},${admin.merchant_admin_id}::Info: Merchant bank account updated successfully addMerchantBankAccount.admin.controllers.merchant.js`);
    }
...
export const validateUnAuthenticatedAdmin = (type = '') => async(req, res, next) => {
  try {
    ...
    if (admin && (admin.status !== 'active' || admin.is_deleted)) {
      ...
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${admin.merchant_admin_id}:::Info: successfully confirms that admin account is ${adminStatus} in the database
