...
export const generateVerificationToken = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: checked if OTP is existing in the database middlewares.auth.js`);
    if (existingOtp) {
      generateVerificationToken(req, res, next);
    }
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully generates random OTP middlewares.auth.js`);
    req.otp = otp;
    return next();
  } catch (error) {
...
export const generateReferralCode = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: checked if referral code previously existed in the db middlewares.auth.js`);
    if (existingReferralCode) {
      generateReferralCode(req, res, next);
    }
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully generated user referral code middlewares.auth.js`);
...
export const checkIfReferralCodeExists = async(req, res, next) => {
  try {
    ...
    if(!body.referral_code) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, Info: referral code is not part of signup payload middlewares.auth.js`);
      return next();
    }
    ...
    if (!referringUserDetails) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully confirms that referral code does not belongs to an existing user middlewares.auth.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully confirms that referral code belongs to an existing user middlewares.auth.js`);
...
export const verifyVerificationToken = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: checked if correct OTP is sent middlewares.auth.js`);
    if (!otpUser) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, Info: OTP is invalid middlewares.auth.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${otpUser.user_id}:::Info: OTP is valid middlewares.auth.js`);
    const isExpired = new Date().getTime() > new Date(otpUser.verification_token_expires).getTime();
    if (isExpired) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${otpUser.user_id}:::Info: successfully confirms that verification token has expired middlewares.auth.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${otpUser.user_id}:::Info: successfully confirms that verification token is still active middlewares.auth.js`);
    req.user = otpUser;
    return next();
  } catch (error) {
...
export const generateTokens = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP},${user.user_id}::: Info: successfully generated access token middlewares.auth.js`);
    const refreshToken = await Hash.generateRandomString(50);
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully generated refresh token middlewares.auth.js`);
    const tokenExpiration = await JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString()).exp;
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully fetched token expiration time middlewares.auth.js`);
    const myDate = new Date(tokenExpiration * 1000);
    const tokenExpireAt = dayjs(myDate);
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully converted time from epoch time to a readable format middlewares.auth.js`);
...
export const getAuthToken = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully extracts token middlewares.auth.js`);
    req.token = token;
    return next();
  } catch (error) {
...
export const validateAuthToken = async(req, res, next) => {
  try {
    ...
    if (decoded.message) {
      ...
      error message middlewares.auth.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${decoded.user_id}:::Info: successfully fetched the users details using the decoded id middlewares.auth.js`);
    if (!user) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully decoded that the user with the decoded id does not exist in the DB middlewares.auth.js`);
    ...
    if (user && (user.is_deleted || user.status === 'suspended' || user.status === 'deactivated')) {
      ...
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${decoded.user_id}:::Info: successfully confirms that user account already exists in the database middlewares.auth.js`);
...
export const isCompletedKyc = async(req, res, next) => {
  try {
    ...
    if (user.is_completed_kyc) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully confirms that user has been previously completed their kyc middlewares.auth.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: user has complete their kyc middlewares.auth.js`);
    return next();
  } catch (error) {
...
export const isPasswordCreated = async(req, res, next) => {
  try {
    ...
    if (user.is_created_password) {
      userActivityTracking(user.user_id, 7, 'fail');
      logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully confirms that user has previously created password middlewares.auth.js`);
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: successfully confirms that user has not previously created password middlewares.auth.js`);
    return next();
  } catch (error) {
    userActivityTracking(req.user.user_id, 7, 'fail');
...
export const checkIfEmailAlreadyExist = async(req, res, next) => {
  try {
    ...
    if (!emailUser) {
      ...
      the database middlewares.auth.js`);
      return next();
    }
