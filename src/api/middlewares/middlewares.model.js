...
const validateData = (schema, type) => async(req, res, next) => {
  try {
    ...
    if (!isValid.error) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully validates request parameters validateData.middlewares.model.js`);
      return next();
    }
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: failed to validate request parameters validateData.middlewares.model.js`);
