...
export const updateEmail = async(req, res, next) => {
  try {
    ...
    const expirationTime = dayjs(expireAt);
    const payload = [ user.email, otp, expireAt ];
    await AuthService.updateUserEmail(payload);
    const data ={ user_id: user.user_id, otp, otpExpire: expirationTime };
    if (SEEDFI_NODE_ENV === 'test') {
      return ApiResponse.success(res, enums.PASSWORD_TOKEN, enums.HTTP_OK, data);
    }
    // email subject to be change later
    MailService('Reset your password', 'forgotPassword', { otp, ...user });
    logger.info(`[${enums.CURRENT_TIME_STAMP}, ${user.user_id},
      Info: email verification has been sent successfully to user mail. updateEmail.controller.auth.js`);
    return ApiResponse.success(res, enums.UPDATE_EMAIL, enums.HTTP_OK);
  } catch (error) {
    error.label = enums.UPDATE_EMAIL_CONTROLLER;
    logger.error(`updating user email failed:::${enums.UPDATE_EMAIL_CONTROLLER}`, error.message);
    return next(error);
  }
};
...
export const verifyEmail = async(req, res, next) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${user.user_id}:::Info: 
    User email address verified in the DB.controller.auth.js`);
