import 'dotenv/config';

export const userOneProfile = {
  first_name: 'Victory',
  middle_name: 'Rashidat',
  last_name: 'Babatunde',
  email: 'victory@enyata.com',
  date_of_birth: '1984-12-13',
  gender: 'female'
};

export const userTwoProfile = {
  first_name: 'Adeleye',
  middle_name: 'Popmping',
  last_name: 'Blaise',
  email: 'adeleye@enyata.com',
  date_of_birth: '1997-10-03',
  gender: 'male'
};

export const userThreeProfile = {
  first_name: 'Elizabeth',
  last_name: 'Agboka',
  email: 'eli@enyata.com',
  date_of_birth: '1954-08-08',
  gender: 'female'
};

export const userFourProfile = {
  first_name: 'Azizat',
  last_name: 'Alausa',
  email: 'azizat@enyata.com',
  date_of_birth: '1954-12-08',
  gender: 'female'
};

export const userSixProfile = {
  first_name: 'Yemisi',
  last_name: 'Ojo',
  email: 'yemisi@enyata.com',
  date_of_birth: '1954-12-08',
  gender: 'female'
};

export const userSevenProfile = {
  first_name: 'Motunrayo',
  last_name: 'Iji',
  email: 'iji@enyata.com',
  date_of_birth: '1954-12-08',
  gender: 'male'
};

export const userEightProfile = {
  first_name: 'Timileyin',
  last_name: 'Olakunle',
  email: 'timi@enyata.com',
  date_of_birth: '1984-12-17',
  gender: 'female'
};

export const userNineProfile = {
  first_name: 'Bala',
  middle_name: 'Andrew',
  last_name: 'Kuye',
  email: 'test@gmail.com', // this email is same that was used to invite a non existing user to join a cluster
  date_of_birth: '1987-12-10',
  gender: 'male'
};

export const userThreeInvalidDateProfile = {
  first_name: 'Elizabeth',
  middle_name: 'Kuye',
  last_name: 'Agboka',
  email: 'eli@enyata.com',
  date_of_birth: '18-08-1954',
  gender: 'female'
};

export const userThreeInvalidEmailProfile = {
  first_name: 'Elizabeth',
  last_name: 'Agboka',
  email: 'victoryenyata.com',
  date_of_birth: '1954-08-08',
  gender: 'female'
};

export const userThreeExistingEmailProfile = {
  first_name: 'Elizabeth',
  last_name: 'Agboka',
  email: 'victory@enyata.com',
  date_of_birth: '1954-09-29',
  gender: 'female'
};
  

