...
describe('', () => {
  ...
  describe('User controller catch block unit testings', () => {
    ...
    it('should call fetchLoanTierValue', async() => {
      const req = { query: '' };
      const data = await UserController.fetchLoanTierValue(req, res, next);
      expect(data.code).to.equal(500);
      expect(data.error).to.equal('INTERNAL_SERVER_ERROR');
    });
    it('should call fetchAlertNotification', async() => {
      const req = { query: '' };
      const data = await UserController.fetchAlertNotification(req, res, next);
      expect(data.code).to.equal(500);
      expect(data.error).to.equal('INTERNAL_SERVER_ERROR');
    });
    ...
    it('should call deleteUserAccount', async() => {
      const req = { user: '' };
      const data = await UserController.deleteUserAccount(req, res, next);
      expect(data.code).to.equal(500);
      expect(data.error).to.equal('INTERNAL_SERVER_ERROR');
    });
  });
