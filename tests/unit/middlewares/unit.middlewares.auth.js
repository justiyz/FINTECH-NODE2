...
import * as AuthMiddleware from '../../../src/api/middlewares/middlewares.auth';
...
describe('', () => {
  ...
  describe('Auth middleware catch block unit testings', () => {
    ...
    it('should call isEmailVerified', async() => {
      const req = { user: '', body: '' };
      const data = await AuthMiddleware.isEmailVerified(req, res, next);
      expect(data.code).to.equal(500);
      expect(data.error).to.equal('INTERNAL_SERVER_ERROR');
    });
  });
